...
DEFINE_SDOF_PROPERTIES(piston_motion, prop, dt, time, dtime)
{
    ...
    // 피스톤의 Y축 위치 설정 (SDOF_POSITION_Y 사용)
    prop[SDOF_POSITION_Y] = position_y;  // Y축 위치

    // 피스톤의 Y축 속도 설정 (SDOF_VELOCITY_Y 사용)
    prop[SDOF_VELOCITY_Y] = velocity_y;  // Y축 속도

    // 물리적 특성 설정 (필요 시 질량, 강성 등)
    prop[SDOF_MASS] = 0.5; // 시스템 질량 (kg)
    prop[SDOF_LOAD_F_Y] = -200.0 * position_y; // Y축에 의한 힘 (필요 시 힘으로 설정)
